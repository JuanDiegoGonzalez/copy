...
public class MVCModelo {
	...
	public Queue A (int mes)
	{
		...
		Node[] temp = new Node[respuesta.darNumeroElementos()];

		for (int i = 0; i < temp.length; i++)
		{
			temp[i] = respuesta.darPrimerNodo();
			respuesta.dequeue();
		}
		...
		for (int i = 0; i < temp.length; i++)
		{
			...
			for (int j = i; j > 0 && !enPos; j--)
			{
				...
				double key1 = datos1[3];
				double key2 = datos2[3];
		...
		for (int i = 0; i < temp.length; i++)
		{
			respuesta.enqueue(temp[i].darDato());
		}		
